...
library(maps)
# include familiarity and filter out artists with familiarity <= 0
artist_coords <- music %>%
  ...
  filter(!is.na(artist.familiarity), artist.familiarity > 0)
...
# color points by familiarity
pal <- colorRampPalette(c("#ffffb2", "#fecc5c", "#fd8d3c", "#f03b20", "#bd0026"))
cols <- pal(100)[as.integer(cut(artist_coords$artist.familiarity, breaks = 100))]

points(artist_coords$artist.longitude, artist_coords$artist.latitude,
  pch = 21, bg = cols, col = "#333333", cex = 0.7)

title(main = "Artist Locations (colored by artist.familiarity)",
      sub = paste("Plotted artists:", n_artists, "— colors show artist.familiarity; artists with familiarity ≤ 0 and placeholder coords (0,0) are excluded."))
#
#
#
#
